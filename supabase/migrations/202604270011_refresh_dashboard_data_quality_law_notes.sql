-- Purpose: refresh dashboard data-quality notes after korean-law-mcp verification pass.
...
with counts as (
  select
    ...
    (select count(*) from public.law_citations where source_type = 'decision_case' and verification_status = 'needs_review')::bigint as needs_review_law_citations,
    (
      select count(*)
...
select
  ...
  '기준일별 조문 검증 완료 ' || verified_law_citations || '건, 재검토 ' || needs_review_law_citations || '건입니다. 재검토는 법령명 추정 또는 항 단위 불일치 후보입니다.'
from counts
union all
select
  'document_chunks',
